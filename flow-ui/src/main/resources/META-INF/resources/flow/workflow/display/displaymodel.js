...
function convertElementPropVal(PropName, PropVal){
    if(PropName === 'Assignee'){
        if(PropVal === '${starter}') {
            ...
            return getAssigneeNameById(PropVal);
        }
    } else if(PropName === 'Candidate groups'){
        return getGroupNameById(PropVal);
    }
    else {
        return '';
    }
...
{
    if(!elementTypeEnum[element.type]){
        return;
    }
    // Custom tooltip
    ...
    if (documentation === undefined) {
        ...
        if (element.properties) {
            ...
            for (var i = 0; i < element.properties.length; i++) {
                if(!elementPropEnum[element.properties[i].name]){
                    continue;
                }
                var propName = elementPropEnum[element.properties[i].name];
                if (element.properties[i].type && element.properties[i].type === 'list') {
                    ...
                    for (var j = 0; j < element.properties[i].value.length; j++) {
                        documentation += '<i>' + convertElementPropVal(element.properties[i].name,element.properties[i].value[j]) + '</i><br/>';
                    }
                }
                else {
                    documentation += '<b>' + propName + '</b>: <i>' + convertElementPropVal(element.properties[i].name,element.properties[i].value) + '</i><br/>';
                }
            }
        }
    }

    var text = elementTypeEnum[element.type]  + " ";
    if (element.name && element.name.length > 0)
    {
        text += element.name;
    }
...
if (modelType == 'runtime') {
    if (historyModelId) {
        modelUrl = FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-instances/history/' + historyModelId + '/model-json';
    } else if (isDebuggerEnabled) {
        modelUrl = FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-instances/debugger/' + modelId + '/model-json';
    } else {
        modelUrl = FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-instances/' + modelId + '/model-json';
    }
    ...
    modelUrl = FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-definitions/' + processDefinitionId + '/model-json';
}
