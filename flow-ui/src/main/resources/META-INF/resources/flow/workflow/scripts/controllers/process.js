...
angular.module('flowableApp')
    ...
    $scope.getProcessInstance = function(processInstanceId) {
        $http({method: 'GET', url: FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-instances' + processInstanceId}).
...
angular.module('flowableApp')
    .controller('ShowProcessDiagramCtrl', ['$scope', '$http', '$interval', '$timeout', '$translate', '$q', 'ResourceService', 'appResourceRoot', '$rootScope',
        function ($scope, $http, $interval, $timeout, $translate, $q, ResourceService, appResourceRoot, $rootScope) {
            var processInstanceId = getUrlParam('procInstId');
            $scope.model = {};
            $http({
                method: 'GET',
                url: FLOWABLE.CONFIG.contextRoot + '/rest/flow/process-instances/' + processInstanceId
            }).
            success(function(response, status, headers, config) {
                $scope.model.processInstance = response;
                ...
                $timeout(function () {
                    ...
                    var viewerUrl = appResourceRoot + "../workflow/display/displaymodel.html?version=" + Date.now();
            ...
            $scope.model.processInstance['id'] = getUrlParam('procInstId');
